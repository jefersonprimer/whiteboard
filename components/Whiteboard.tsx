...
const Canvas = lazy(() =>
  import("./Canvas").then((mod) => ({ default: mod.Canvas })),
);

const TOOLS_WITH_CLICK_TOOLTIP = new Set<Tool>([
  "select",
  "arrow",
  "line",
  "pencil",
...
export default function Whiteboard() {
  ...
  const { elements, setElements, saveHistory, undo, redo, canUndo, canRedo } =
    useHistoryState([]);
  ...
  const [isLiveCollaborationModalOpen, setIsLiveCollaborationModalOpen] =
    useState(false);
  ...
  const TOOL_CLICK_TOOLTIP: Partial<Record<Tool, React.ReactNode>> = {
    select: (
      <>
        ...
        <span className="p-0.5 border rounded">
          {t("toolClickTooltip.select.scrollWheelKey")}
        </span>{" "}
        {t("toolClickTooltip.select.middle")}{" "}
        <span className="p-0.5 border rounded">
          {t("toolClickTooltip.select.spaceKey")}
        </span>{" "}
    ...
    eraser: (
      <>
        ...
        <span className="p-0.5 border rounded">
          {t("toolClickTooltip.eraser.altKey")}
        </span>{" "}
  ...
  // Clear selection when switching to a drawing tool so properties apply to the next stroke/shape
  useEffect(() => {
  ...
  const handleReplaceElements = useCallback(
  ...
  const handleImageUpload = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Reset so the same file can be selected again on the next click
      e.target.value = "";
      if (!file) return;
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const id = nanoid();
          const centerX = (-stagePosition.x + window.innerWidth / 2) / zoom;
          const centerY = (-stagePosition.y + window.innerHeight / 2) / zoom;
    ...
    [saveHistory, defaultProps, stagePosition, zoom],
  );

  const updateElements = useCallback(
    async (updates: Partial<WhiteboardElement>) => {
      // Only modify existing elements when using the selection tool
        ...
        if (
          ...
          if (targetN !== currentN) {
            ...
            for (let i = 0; i < targetN; i++) {
              ...
              const y =
                (pts[lo * 2 + 1] ?? 0) * (1 - f) + (pts[hi * 2 + 1] ?? 0) * f;
              out.push(x, y);
            }
            merged.points = out;
          }
    ...
    [activeTool, selectedIds, saveHistory],
  );

  const handleLayerChange = useCallback(
    async (action: "front" | "back" | "forward" | "backward") => {
      ...
      if (action === "front") {
        ...
        for (let i = selectedIndices.length - 1; i >= 0; i--) {
          ...
          if (
            ...
            !selectedIds.includes(newElements[idx + 1].id)
          ) {
    ...
    [activeTool, elements, selectedIds, saveHistory],
  );
    ...
    >
      ...
      <div className="fixed top-4 left-4 z-[100]">
        <button
          ...
          className="p-2 bg-white dark:bg-[#1C1C1C] border border-gray-200 dark:border-neutral-800 rounded-lg shadow-lg hover:bg-gray-50 dark:hover:bg-neutral-800 text-gray-700 dark:text-white transition-all active:scale-95 hover:cursor-pointer"
          title="Menu"
        >
      ...
      >
        ...
        />
        <div
          className={`absolute top-16 right-4 transition-all duration-200 ease-out origin-top-right ${
      ...
      {clickTooltipTool !== null &&
        ...
        createPortal(
          <div className="hidden md:block fixed top-18 left-1/2 -translate-x-1/2 z-50 w-[min(900px,calc(100vw-2rem))] pointer-events-none">
