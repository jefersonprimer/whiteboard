...
const WebEmbedYoutube: React.FC<WebEmbedMediaProps> = ({ width, height }) => {
...
const WebEmbedVimeo: React.FC<WebEmbedMediaProps> = ({ width, height }) => {
  const contentWidth = width - 20;
  const contentHeight = height - 20;
  return (
    <Rect x={10} y={10} width={contentWidth} height={contentHeight} fill="transparent" />
...
const WebEmbedVideo: React.FC<WebEmbedMediaProps> = ({ width, height }) => {
  const contentWidth = width - 20;
  const contentHeight = height - 20;
  return (
    <Rect x={10} y={10} width={contentWidth} height={contentHeight} fill="transparent" />
...
function getPointerPressure(e: { evt?: MouseEvent | TouchEvent | PointerEvent }): number {
  const evt = e?.evt;
  if (!evt) return 0.5;
...
const ImageElement = ({ el, activeTool, onDragEnd, onTransformEnd, onClick }: any) => {
  const crossOrigin =
    el.src?.startsWith('data:') || el.src?.startsWith('blob:')
      ? undefined
      : 'anonymous';
  const [img] = useImage(el.src, crossOrigin);
  return (
    <KonvaImage
      id={el.id}
      image={img}
...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  const [, setDraggingControlPoint] = useState<{ elementId: string; pointIndex: number } | null>(null);
  ...
  const handleTextInput = useCallback((x: number, y: number, id: string, initialText = '', onEditEnd?: () => void) => {
    ...
    const existingEl = elementsRef.current.find((el) => el.id === id);
    const textFontSize =
      existingEl?.type === 'text'
        ? (existingEl.fontSize ?? defaultProps.fontSize ?? 20)
        : (defaultProps.fontSize || 20);

    textarea.style.top = top + 'px';
    textarea.style.left = left + 'px';
    // Match Konva Text screen size: scene fontSize × stage zoom (no extra CSS transform).
    textarea.style.fontSize = `${textFontSize * scale}px`;
    ...
    const finishText = async () => {
      ...
      if (val.trim()) {
        const element: WhiteboardElement = {
          ...
          opacity: 1,
          ...defaultProps,
          fontFamily: existingEl?.fontFamily ?? defaultProps.fontFamily ?? 'Sans-serif',
          fontSize: textFontSize,
          textAlign: existingEl?.textAlign ?? defaultProps.textAlign ?? 'left',
        ...
        if (existingIndex !== -1) {
          ...
          newArr[existingIndex] = { ...currentElements[existingIndex], ...element };
          saveHistory(newArr);
        } else {
  ...
  const handleMouseDown = useCallback((e: any) => {
    ...
    const element: WhiteboardElement = {
      ...
      ...(activeTool === 'pencil' && { points: [0, 0], pointWidths: [] }),
    };

    if (activeTool === 'pencil') {
      const brushSize = element.strokeWidth ?? defaultProps.strokeWidth ?? 2;
      const brush = new VelocityBrushEngine(createVelocityBrushConfig(brushSize));
      brush.addPoint(0, 0, performance.now(), getPointerPressure(e));
      pencilBrushRef.current = brush;
      const strokeData = brush.getStrokeData();
      element.points = strokeData.points;
      element.pointWidths = strokeData.pointWidths;
    } else {
  ...
  const handleMouseMove = useCallback((e: any) => {
    ...
    if (updatedElement.type === 'rectangle' || updatedElement.type === 'circle' || updatedElement.type === 'triangle' || updatedElement.type === 'diamond' || updatedElement.type === 'frame') {
      ...
      const relX = pos.x - currentNew.x;
      const relY = pos.y - currentNew.y;
      const brush = pencilBrushRef.current;
      if (brush) {
        brush.addPoint(relX, relY, performance.now(), getPointerPressure(e));
        const strokeData = brush.getStrokeData();
        updatedElement.points = strokeData.points;
        updatedElement.pointWidths = strokeData.pointWidths;
      } else {
        updatedElement.points = [...(currentNew.points || []), relX, relY];
      }
    }
  ...
  const handleMouseUp = useCallback(async () => {
    ...
    if (currentNew) {
      ...
      if (finalElement.type === 'pencil' && pencilBrushRef.current) {
        const strokeData = pencilBrushRef.current.finalize();
        finalElement.points = strokeData.points;
        finalElement.pointWidths = strokeData.pointWidths;
        pencilBrushRef.current = null;
      }
  ...
  const handleTransformEnd = useCallback(() => {
    ...
    for (const node of nodes) {
      ...
      if (element.type === 'rectangle' || element.type === 'text' || element.type === 'image') {
        ...
        const scaleX = node.scaleX();
        const scaleY = node.scaleY();
        const scaledPoints = pts.map((p, idx) => (idx % 2 === 0 ? p * scaleX : p * scaleY));
        updatedElement.points = scaledPoints;
        const thicknessScale = (Math.abs(scaleX) + Math.abs(scaleY)) / 2;
        if (element.pointWidths?.length) {
          updatedElement.pointWidths = element.pointWidths.map((w) =>
            Math.max(updatedElement.strokeWidth || 1, w * thicknessScale),
          );
        }
        const currentStrokeWidth = element.strokeWidth || 2;
            ...
            <>
              ...
              {newElement.type === 'pencil' && (
                <PencilStroke
                  ...
                  pointWidths={newElement.pointWidths || []}
                  stroke={resolveStroke(newElement.stroke)}
                  opacity={newElement.opacity ?? 0.5}
                />
              )}
            </>
